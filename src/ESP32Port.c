...
// this routes the SPI signals through the GPIO MUX, hence will be slower, the clock will only be able to run up to 40
// MHz #define GPIO_SCLK      42   // D13 in Arduino UNO terms #define GPIO_MISO      21   // D12 in Arduino UNO terms
...
#define GPIO_SCLK 12  // 18 D13 in Arduino UNO terms
#define GPIO_MISO 13  // 19 D12 in Arduino UNO terms
#define GPIO_MOSI 11  // 23 D11 in Arduino UNO terms
#define GPIO_CS \
  10  // *5 D10 in Arduino UNO terms, this is a dummy pin that will not be used (AD5940 library was programmed in such a
      // way that chip selects are handled manually)
#define AD5940_CS_PIN 16      // 16 this is the true CS pin, AD5940 will not work with the default CS pin.
#define AD5940_GP0INT_PIN 14  // 14 D2 in Arduino UNO terms, this connects to GPIO0 of AF5940
#define AD5940_RST_PIN 17     // 17 A3/D17 in Arduino UNO terms
...
uint32_t AD5940_MCUResourceInit(void *pCfg) {
  ...
  gpio_config_t adf5940_rst_conf = {
      .intr_type = GPIO_INTR_DISABLE, .mode = GPIO_MODE_OUTPUT, .pull_up_en = 1, .pin_bit_mask = (1 << AD5940_RST_PIN)};

  // GPIO config for the interrupt pin of AD5940
  gpio_config_t ad5940_int_conf = {.intr_type = GPIO_INTR_NEGEDGE,  // the interrupt triggers on a falling edge
  ...
  gpio_config_t ad5940_cs_conf = {
      .intr_type = GPIO_INTR_DISABLE, .mode = GPIO_MODE_OUTPUT, .pull_up_en = 1, .pin_bit_mask = (1 << AD5940_CS_PIN)};
  ...
  gpio_set_level(
      AD5940_CS_PIN,
      1);  // pull CS high, there were scenarios where this was pulled low despite it being defined as pull-up
